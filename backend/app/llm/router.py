...
def build_llm_prompt(text: str, schema: Dict[str, Any], known_names: List[str]) -> str:
    ...
    return (
        ...
        "ВАЖНО: Верни ТОЛЬКО одно действие. Никогда не добавляй лишних действий.\n"
        "  Если пользователь сказал «создай задачу X для Y» — верни только add_task. Не добавляй reassign после него.\n"
        "  Если пользователь сказал «поставь на N день» — верни только set_start_day.\n"
        "Примеры:\n"
