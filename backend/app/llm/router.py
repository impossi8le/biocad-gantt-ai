...
def build_llm_prompt(text: str, schema: Dict[str, Any], known_names: List[str]) -> str:
    ...
    return (
        ...
        '{"action": "<инструмент>", "targets": {"tasks": ["имя"] или "all"}, '
        '"params": {...}, "explanation": "что сделано (русский)"}\n\n'
        "Доступные действия:\n"
        "- get_schema — схема плана. params: {}\n"
        "- get_task — детали задачи. params: {task: str}\n"
        "- shift_tasks — сдвиг задач. params: {mode: 'offset'|'to_date', value: int}\n"
        "- set_dependency — зависимость. params: {task: str, depend_on: str}\n"
        "- add_task — добавить задачу. params: {name: str, assignee: str, duration_days: int, description: str, predecessors: list}\n"
        "- reassign — сменить исполнителя. params: {new_assignee: str}\n"
        "- update_field — обновить поле. params: {field: str, value: str}\n"
        "- remove_tasks — удалить задачи. params: {}\n"
        "- set_start_day — установить день старта. params: {task: str, day: int}\n"
        "- compute — статистика. params: {agg: str, field: str, by: str}\n"
        "- export — экспорт. params: {format: 'xlsx'|'csv'}\n"
        "- undo — откат. params: {}\n"
        "- help — справка. params: {}\n\n"
        f"План: {json.dumps(schema, ensure_ascii=False)}\n"
        f"Задачи: {', '.join(known_names) or '(нет)'}.\n"
        f"Запрос: {text}\n\n"
        "Правила:\n"
        "- Одно действие за раз.\n"
        "- Имена (исполнителей, задач) извлекай из запроса. Нормализуй уменьшительные формы (Дашутка→Дарья).\n"
        "- Если запрос составной — верни ТОЛЬКО первое действие. Остальное будет обработано следом.\n"
    )
